...
class CustomerAdmin(admin.ModelAdmin):
    list_display = ('name', 'phone_number', 'area', 'date', 'status', 'assigned_to', 'created_at')
    list_filter = ('status', 'assigned_to', 'date')
    search_fields = ('name', 'phone_number', 'area')
    ...
    def get_fieldsets(self, request, obj=None):
        fieldsets = [
            (None, {
                'fields': ('name', 'phone_number', 'area', 'date', 'status', 'assigned_to', 'remark', 'notes')
            }),
        ]
        return fieldsets
